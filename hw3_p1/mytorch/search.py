...
def init_paths(symbols, prob):
    path_symbol_score, path_blank_score = {}, {}
...
def prune(path_blank_score, path_symbol_score, beam_width):
    ...
    pruned_path_blank_score = {}
    pruned_path_symbol_score = {}
...
def merge_path(path_blank_score, path_symbol_score):
    ...
    for path, score in path_blank_score.items():
        # merge path score if possible
        if path in merged_path:
...
def GreedySearch(SymbolSets, y_probs):
    ...
    new_path_blank_score, new_path_symbol_score = init_paths(SymbolSets, y_probs[:,0,-1])

    for i in range(1, y_probs.shape[1]):
        prob = y_probs[:,i,-1]
        path_blank_score, path_symbol_score = prune(new_path_blank_score, new_path_symbol_score, beam_width=1)
    ...
    path, path_prob = sorted(merged_path.items(), key=lambda x: x[1], reverse=True)[0]
