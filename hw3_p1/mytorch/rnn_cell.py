...
class RNN_Cell(object):
    ...
    def forward(self, x, h):
        ...
        """
        h_prime = self.activation(np.matmul(x, self.W_ih.T) + self.b_ih + np.matmul(h, self.W_hh.T) + self.b_hh)
        return h_prime
    ...
    def backward(self, delta, h, h_prev_l, h_prev_t):
        ...
        self.dW_ih += np.matmul(h_prev_l.T, dz).T / batch_size
        self.dW_hh += np.matmul(h_prev_t.T, dz).T / batch_size
